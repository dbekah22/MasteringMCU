...
void I2C_MasterReceiveData(I2C_Handle_t *pI2CHandle, uint8_t *pRXBuffer, uint32_t Len, uint8_t SlaveAddr, uint8_t Sr)
{
	...
	if (Len == 1)
	{
		...
		I2C_ClearADDRFlag(pI2CHandle);									// Clear ADDR flag for reception to begin
 ...
 *
 * @brief		- This function allows slave to send data to master
 *
 * @pI2Cx		- Base address pointer to I2C peripheral
 * @data		- Data to be loaded into DR
 *
 * @return		- none
 *
 * @Note		- none
 ...
 *
 * @brief		- This function returns data received by slave from master
 *
 * @pI2Cx		- Base address pointer to I2C peripheral
 *
 * @return		- data (uint8_t)
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
	...
	{
		if (pI2CHandle->pI2Cx->SR2 & (1 << I2C_SR2_MSL))	// Check device mode before proceeding
		{	// Master mode
		...
		{	// Slave mode
			if (pI2CHandle->pI2Cx->SR2 & (1 << I2C_SR2_TRA))	// Make sure slave is in transmitter mode (TRA=1)
	...
	if (temp1 && temp2 && temp3)
	{
		...
		{	// Slave mode
			if (!(pI2CHandle->pI2Cx->SR2 & (1 << I2C_SR2_TRA)))	// Make sure slave is in receiver mode (TRA=0)
